...
flextable((Table %>% tibble::rownames_to_column(" ")), 
          col_keys = c(" ", 
                       "Common.survival.analysis", 
                       "Analogs.in.trip.decay")) %>%
  empty_blanks() %>%
  bold(part = "header") %>%
  bold(j    = 1) %>%
  set_header_labels(Common.survival.analysis = "Common Survival Analysis", 
                    Analogs.in.trip.decay    = "Analogs in Trip Decay") %>%
...
Table <- 
  data.frame("Model" = rep(c("Personal vehicle",
             ...
             "Total Records"=prettyNum(rep(c(112262, 26137), 
                                           c(5,3)), big.mark=","))
border_table = fp_border(color = "black", width = 2)

flextable(Table, 
          col_keys = c("Model", "Modes", "Variable", 
                       "Filter.Condition", "Total.Records")) %>%
  bold(part = "header") %>%
  set_header_labels(Filter.Condition = "Filter Condition", 
                    Total.Records    = "Total Records") %>%
  set_caption(caption = "Table 2: Trip record filtering protocol for each mode-specific model") %>%
  align(align = "left", part = "all") %>%
  merge_v(j = ~ Model + Modes + Total.Records) %>%
  border_inner_h(part = "body", border = border_table) %>%
  fix_border_issues() %>%
  autofit()
```
...
flextable(Table, 
          col_keys = c("Model", "Covariates")) %>%
  bold(part = "header") %>%
  set_caption(caption = "Table 3: Candidate covariates considered for each mode-specific model") %>%
  align(align = "left", part = "all") %>%
  merge_v(j = ~ Model) %>%
  border_inner_h(part = "body", border = border_table) %>%
  fix_border_issues() %>%
...
flextable(Table, col_keys = c("Purpose.Group", "MTS.Purpose.Code",
                              "MTS.Purpose.Definition")) %>%
  bold(part = "header") %>%
  set_caption(caption = "Table 4: Methodology for collapsing purpose to home, work, and non-work groups") %>%
  set_header_labels(Purpose.Group          = "Purpose Group", 
                    MTS.Purpose.Code       = "MTS Purpose Code",
                    MTS.Purpose.Definition = "MTS Purpose Definition") %>%
  align(align = "left", part = "all") %>%
  merge_v(j = ~ Purpose.Group) %>%
  border_inner_h(part = "body", border = border_table) %>%
  fix_border_issues() %>%
  autofit()
...
s$coefficients %>%
  ...
  add_column("Covariate"=c("Parking cost", "Toll cost", "Household size", 
             ...
             .before=1) %>%
  flextable() %>%
  set_caption("Table 5: Coefficients for initial personal vehicle model") %>%
  autofit()
...
s$conf.int %>%
  ...
  add_column("Covariate"=c("Parking cost", "Toll cost", "Household size", 
             ...
             .before=1) %>%
  flextable() %>%
  set_caption("Table 6: Hazard ratios (HR) for initial personal vehicle model") %>%
  autofit()
...
test$table %>%
  ...
  add_column("Covariate"=c("Parking cost", "Toll cost", "Household size", 
                           ...
                           "Destination TAZ terminal time",
                           "Global"), .before=1) %>%
  flextable(col_keys = c("Covariate", "X2", "p")) %>%
  set_caption("Table 7: Results of Schoenfeld Residuals Tests for initial personal vehicle model") %>%
  autofit()
```
...
s$coefficients %>%
  ...
  add_column("Interval"=rep(c("0-5 minutes", "5-10 minutes", "10-20 minutes", 
             ...
             .before=2) %>%
  flextable() %>%
  set_caption("Table 8: Coefficients for time-dependent personal vehicle model") %>%
  autofit()
```
...
s$conf.int %>%
  ...
  add_column("Interval"=rep(c("0-5 minutes", "5-10 minutes", "10-20 minutes", 
             ...
             .before=2) %>%
  flextable() %>%
  set_caption("Table 9: Hazard ratios (HR) for time-dependent personal vehicle model") %>%
  autofit()
```
...
```{r, "Time-dependent personal vehicle model deviance residuals", fig.pos='H', out.extra = ''}
...
```{r, fig.cap="Difference in exampel model and sample survival probabilities for personal vehicle travel", fig.pos='H', out.extra = '', eval=FALSE, echo=TRUE}
together %>%
  spread(Curve, y) %>%
...
s$coefficients %>%
  ...
  add_column("Covariate"=c("Destination TAZ parking",
             ...
             .before=1) %>%
  flextable() %>%
  set_caption("Table 10: Coefficients for initial non-motorized model") %>%
  autofit()
```
...
s$conf.int %>%
  ...
  add_column("Covariate"=c("Destination TAZ parking",
             ...
             .before=1) %>%
  flextable() %>%
  set_caption("Table 11: Hazard ratios (HR) for initial non-motorized model") %>%
  autofit()
```
...
test %$%
  ...
  add_column("Covariate"=c("Destination TAZ parking",
             ...
             .before=1) %>%
  flextable(col_keys = c("Covariate", "X2","p")) %>%
  set_caption("Table 12: Results of Schoenfeld Residuals Tests for initial non-motorized model") %>%
  autofit()
```
...
s$coefficients %>%
  ...
  add_column("Interval"=rep(c("0-5 minutes", "> 5 minutes"), times=4),
             .before=2) %>%
  flextable() %>%
  set_caption("Table 13: Coefficients for time-dependent non-motorized model") %>%
  autofit()
```
...
s$conf.int %>%
  ...
  add_column("Interval"=rep(c("0-5 minutes", "> 5 minutes"), times=4),
             .before=2) %>%
  flextable() %>%
  set_caption("Table 14: Coefficients for time-dependent non-motorized model") %>%
  autofit()
...
```{r,  fig.cap="Time dependent non-motorized model deviance residuals", fig.pos='H', out.extra = ''}
devres = residuals(fitcut, type="deviance")
ggplot(data = data.frame(y = devres, x = 1:length(devres))) +
  geom_point(aes(x = x, y = y)) +
  geom_hline(yintercept=0, color="red") +
...
```{r, fig.cap="Example comparison of model and sample travel time decay curves for non-motorized travel", fig.pos='H', out.extra = '', eval=FALSE, echo=TRUE}
...
flextable(cases)%>%
  set_caption("Table 15: Coefficients for C0 and CAi, i = 1, 2, 3") %>%
  autofit()
```
...
```{r, fig.cap="$S_{C_0}(t)$, with $S_{C_0}(14)$ highlighted", fig.pos='H', out.extra = '', eval=FALSE, echo=TRUE}
...
kable(totprob %>% mutate(pdf = round(pdf, 3), d = round(d, 3)),
      booktabs = T,
      row.names = F,
      col.names = c("Case", "Density", "Density ratio (to $\\bm{C_0}$)"),
      escape = F,
      caption = "$f_{C_0}(t)$, $f_{C_{A_i}}(t)$, and $\\frac{f_{C_{A_i}}(15)}{f_{C_0}(15)}$ for 15 minute vehicle trips") %>%
 # column_spec(1:ncol(df), width = "5cm") %>%
  kable_styling(latex_options = "HOLD_position") %>%
  row_spec(0, bold=T)
```

```{r, fig.cap="$f_{C_0}(t)$ and $f_{C_{A_i}}(t), i=1,2,3$", fig.pos='H', out.extra = '', echo=TRUE, eval=FALSE}
...
```{r,  eval=FALSE, echo=TRUE}
finaldf = totprob %>% 
  mutate(s = df %>% filter(curve == "$C_{0}$" & time == 14) %$% surv) %>%
  mutate(f = d * s) %>%
  select(-pdf) %>%
  slice(-1) %>%
...
kable(finaldf %>% mutate(d = round(d, 3), s = round(s, 3), f = round(f, 3)),
      ...
      escape = F,
      caption = "Calculation of $D_{15,C_0,C_{A_i}}$") %>% 
 # column_spec(1:ncol(df), width = "5cm") %>%
  kable_styling(latex_options = "HOLD_position") %>%
  row_spec(0, bold=T)
```
