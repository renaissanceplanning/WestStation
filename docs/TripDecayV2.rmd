...
s$coefficients %>%
  ...
  tibble::add_column("Covariate"=c("Parking cost", "Toll cost", "Household size", 
...
s$coefficients %>%
  ...
  select(-exp.coef.) %>%
  setNames(c("β", "SE(β)", "z", "Pr(>|z|)")) %>%
  tibble::add_column("Covariate"=rep(c("Parking cost", "Toll cost", "Household size", 
                               "Purpose: work", "Purpose: non-work",
                               "Origin TAZ terminal time",
                               "Destination TAZ terminal time"), each=5),
             .before=1) %>%
  tibble::add_column("Interval"=rep(c("0-5 minutes", "5-10 minutes", "10-20 minutes", 
...
s$coefficients %>%
  ...
  select(-exp.coef.) %>%
  setNames(c("β", "SE(β)", "z", "Pr(>|z|)")) %>%
  tibble::add_column("Covariate"=c("Destination TAZ parking",
                           "Destination TAZ terminal time",
                           "Purpose: work", 
                           "Purpose: non-work"),
             .before=1) %>%
...
s$coefficients %>%
  ...
  select(-exp.coef.) %>%
  setNames(c("β", "SE(β)", "z", "Pr(>|z|)")) %>%
  tibble::add_column("Covariate"=rep(c("Destination TAZ parking",
                               "Destination TAZ terminal time",
                               "Purpose: work", 
                               "Purpose: non-work"), each=2),
             .before=1) %>%
  tibble::add_column("Interval"=rep(c("0-5 minutes", "> 5 minutes"), times=4),
             .before=2) %>%
...
decay_cases = function(base_case, ...){
  ...
  cases = suppressWarnings(
    lapply(1:length(individuals), function(x){
      ...
      tibble::add_column(Case=c("C0","CA1","CA2","CA3"),
                 .before=1)
  )
  return(cases)
}
